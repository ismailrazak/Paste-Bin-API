...
def entry_view(request):
    return Response(data={"users":request.build_absolute_uri(reverse("users")),"snippets":request.build_absolute_uri(reverse("snippets"))})
...
class PasswordView(APIView):
    ...
    def post(self,request,pk):
        ...
        if check_password(password,snippet.password):
            ...
            cache.set(f'access_token_{pk}', token, timeout=300)
            return Response(status=status.HTTP_201_CREATED,data=f'password_token:{token}')
        else:
...
class SnippetViewDetail(UpdateModelMixin,DestroyModelMixin,APIView):
    def get(self,request,pk):
        ...
        if checks:
            return checks
        serializer = serializers.SnippetSerializer(snippet,context={"request":request})
...
class HomeSnippetView(generics.ListCreateAPIView):
    permission_classes = HomeViewPermission,
    serializer_class = serializers.SnippetSerializer

    def get_queryset(self):
        return models.Snippet.objects.filter(author=self.request.user)


class HomesnippetViewDetail(generics.RetrieveUpdateDestroyAPIView):
    permission_classes = HomeViewPermission,
    serializer_class = serializers.SnippetSerializer
    queryset = models.Snippet.objects.all()
...
def _checks(request,snippet,pk):
    ...
    if snippet.view_once:
        if models.OneTimeCode.objects.filter(code=pk).exists():
            snippet.delete()
            return Response(status=HTTP_204_NO_CONTENT, data={"Snippet does not exist."})
        else:
